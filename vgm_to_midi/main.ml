...
let process device machine l =
  ...
  let rec aux l cycles w =
    ...
    | 0 -> (
        match l with
        | Vgm.Parser.Wait n :: xs -> aux xs (n * per_sample) w
        | Write (n, r) :: xs ->
            Gb.Mmu.put machine (n + 0xFF10) (Char.unsafe_chr r);
            let now = Mtime_clock.now_ns () in
            let elapsed = Int64.sub now w in

            if elapsed < cy then aux xs 0 w
            else (
              if machine.sc1.frequency > 30 then (
                let note_sc1 =
                  machine.sc1.frequency |> Vgm.Sound_table.map_sound_to_midi
                  |> Vgm.Uint8.inj
                in
              ...
              if machine.sc2.frequency > 30 then (
                let note_sc2 =
                  machine.sc2.frequency |> Vgm.Sound_table.map_sound_to_midi
                  |> Vgm.Uint8.inj
                in
              ...
              if machine.sc3.frequency > 30 then (
                let note_sc3 =
                  machine.sc3.frequency |> Vgm.Sound_table.map_sound_to_midi
                  |> Vgm.Uint8.inj
                in
...
let load_file path =
  ...
  Bos.OS.File.read path >>= fun content -> Ok content
...
let main () =
  ...
  let* device =
    match
      Portmidi.open_output ~device_id:5 ~buffer_size:Int32.zero
        ~latency:Int32.zero
    with
  ...
  | Ok l ->
      process device machine l;
      turn_off_everything device;
      turn_off_everything device;
      Ok ()

let () =
  match main () with Error (`Msg err) -> print_endline err | Ok () -> ()
